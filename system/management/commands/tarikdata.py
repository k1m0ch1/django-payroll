...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		for z in msn:
		 	...
		 	x = Mesin.objects.select_for_update().filter(id=z.id)
		 	if response.ret_code == 0:		    
			    x.update(status="UP", last_up = datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'), last_check = datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S') )
		 	else:
		 	    x.update(status="DOWN", last_down = datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S'), last_check = datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S') ) 
